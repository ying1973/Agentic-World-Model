...
from awm.tools import Tool, get_tools, synthesis_chatgpt, synthesis_qwen3vl, get_tools_new
...
Begin your first <think> block now."""
    ...
    def _call_llm(self, prompt: str, image: Optional[Any], history: str) -> str:
        media_ids, video_ids = self._extract_media_and_video_ids_from_history(history)
        image_payload = self._build_media_payload_for_llm(media_ids, video_ids)
        ...
        response = synthesis_qwen3vl(prompt, images=image_payload)
        return response.strip()
    ...
    def _parse_response(self, text: str) -> Tuple[Optional[str], Optional[Dict], Optional[str], Optional[str]]:
        ...
        if action_match:
            ...
            action_json = '\n'.join(line.strip() for line in action_json.split('\n'))
    ...
    def run(self, question: str, image: Optional[Any] = None) -> Tuple[str, Dict]:
        ...
        for iteration in range(self.max_iterations):
            ...
            response = self._call_llm(prompt, image, history)
            ...
            try:
                ...
                found_image_ids = set(re.findall(r'(media_[a-fA-F0-9]+)', response))
                found_video_ids = set(re.findall(r'(video_[a-fA-F0-9]+)', response))
                ...
                for mid in list(found_image_ids):
                    ...
                    print("模型作为参数调用的图片的临时路径是",local_path)
            ...
            if action_dict and 'tool' in action_dict and 'input' in action_dict:
                ...
                #     eval_response = self._call_llm(eval_prompt, image, history)
