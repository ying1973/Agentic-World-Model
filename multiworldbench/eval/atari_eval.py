...
def get_corresponding_gen_filename(ref_filename, gen_file_info_dict):
    """
    解析ref文件名的核心三要素（游戏名、样本id、action），从gen文件索引中匹配对应文件
    忽略action序号是否存在，实现精准一一匹配
    :param ref_filename: ref目录下的文件名
    :param gen_file_info_dict: 预构建的gen文件索引字典，键为(游戏名, 样本id, action)，值为gen文件名
    :return: 匹配的gen文件名，无匹配返回None
    """
    # 正则匹配ref文件名，提取核心三要素（忽略action序号是否存在）
    # 匹配格式：{游戏名}-{样本id}-curr-{action}(-{action序号})?.png
    ref_pattern = r'^(.+)-([^-]+)-curr-([^-]+)(?:-\d+)?\.png$'
    match = re.match(ref_pattern, ref_filename)
    if not match:
        return None
    
    game_name, sample_id, action = match.groups()
    # 查找gen文件索引中对应的文件
    key = (game_name, sample_id, action)
    return gen_file_info_dict.get(key, None)
...
def evaluate(
    ...
    # 获取ref_dir下的文件并排序，作为基准列表
    ref_files = sorted(os.listdir(ref_dir))
    # 过滤有效图片文件并确保是curr类型
    image_suffixes = ('.jpg', '.jpeg', '.png', '.bmp', '.tif', '.tiff')
    ref_files = sorted([
        f for f in ref_files
        if f.lower().endswith(image_suffixes) and "-curr-" in f
    ])

    print(f"Found {len(ref_files)} reference images in {ref_dir}")

    # 预构建gen文件索引字典（核心修改：提升匹配效率，忽略action序号）
    gen_file_info_dict = {}
    gen_files = [f for f in os.listdir(gen_dir) if f.lower().endswith(image_suffixes) and "-pred-" in f]
    for gen_fname in gen_files:
        # 正则匹配gen文件名，提取核心三要素（忽略action序号是否存在）
        gen_pattern = r'^(.+)-([^-]+)-pred-([^-]+)(?:-\d+)?\.png$'
        match = re.match(gen_pattern, gen_fname)
        if match:
            game_name, sample_id, action = match.groups()
            key = (game_name, sample_id, action)
            # 若存在多个匹配项，告警并保留第一个（避免重复匹配）
            if key in gen_file_info_dict:
                print(f"[WARN] Multiple gen files match key {key}: {gen_fname} and {gen_file_info_dict[key]}, keep the first one")
                continue
            gen_file_info_dict[key] = gen_fname
    print(f"Built index for {len(gen_file_info_dict)} valid generated images in {gen_dir}")
    ...
    # ---------- Pairwise metrics ----------
    for ref_fname in tqdm(ref_files, desc="Computing LPIPS / SSIM / PSNR"):
        # 获取对应的生成文件名（传入预构建的索引字典）
        gen_fname = get_corresponding_gen_filename(ref_fname, gen_file_info_dict)
        if not gen_fname:
            print(f"[WARN] Could not find corresponding gen file for: {ref_fname} (skipped)")
            continue

        ref_path = os.path.join(ref_dir, ref_fname)
        gen_path = os.path.join(gen_dir, gen_fname)

        # 检查gen_dir中是否存在对应文件，不存在则告警并跳过
        if not os.path.exists(gen_path):
            print(f"[WARN] Missing generated image: {gen_fname} (corresponding to {ref_fname}, skipped)")
            continue
        ...
        # 记录有效文件名，用于后续FID计算
        valid_ref_filenames.append(ref_fname)
        valid_gen_filenames.append(gen_fname)

    # 检查是否有有效匹配的文件
    if len(valid_ref_filenames) == 0:
        ...
        return {
            ...
            "FID (↓)": float('nan'),
            "Valid Image Pairs": 0,
            "Total Reference Images": len(ref_files)
        }

    print(f"Successfully processed {len(valid_ref_filenames)} valid image pairs")
    ...
    # 仅复制有效匹配的文件到临时FID目录
    for ref_fname, gen_fname in zip(valid_ref_filenames, valid_gen_filenames):
        # 复制参考图片
        src_ref = os.path.join(ref_dir, ref_fname)
        dst_ref = os.path.join(fid_ref_dir, ref_fname)  # 保持原名便于追踪
        img = Image.open(src_ref).convert("RGB")
        img = img.resize(fid_resize, Image.Resampling.LANCZOS)
        img.save(dst_ref)

        # 复制生成图片
        src_gen = os.path.join(gen_dir, gen_fname)
        dst_gen = os.path.join(fid_gen_dir, gen_fname)  # 保持原名便于追踪
    ...
    results = {
        ...
        "Valid Image Pairs": len(valid_ref_filenames),
        "Total Reference Images": len(ref_files),
        "LPIPS Std": float(np.std(lpips_scores)),
        "SSIM Std": float(np.std(ssim_scores)),
        "PSNR Std": float(np.std(psnr_scores))
    }
    ...
    print(f"Valid matched images: {len(valid_ref_filenames)}")
    print(f"Missing/Unmatched generated images: {len(ref_files) - len(valid_ref_filenames)}")
    print(f"LPIPS (lower is better): {results['LPIPS (↓)']:.4f} (std: {results['LPIPS Std']:.4f})")
    print(f"SSIM (higher is better): {results['SSIM (↑)']:.4f} (std: {results['SSIM Std']:.4f})")
    print(f"PSNR (higher is better): {results['PSNR (↑)']:.2f} (std: {results['PSNR Std']:.2f})")
...
def compute_gpt_score_for_dirs(
    ...
    """
    对两个目录下的图像进行 GPT 评分
    根据核心三要素（游戏名、样本id、action）匹配，忽略action序号
    """
    # 过滤有效图片文件并确保是curr类型
    gt_files = sorted([
        f for f in os.listdir(gt_dir)
        if f.lower().endswith(suffixes) and "-curr-" in f
    ])

    # 预构建gen文件索引字典（和evaluate函数保持一致的匹配逻辑）
    gen_file_info_dict = {}
    gen_files = [f for f in os.listdir(gen_dir) if f.lower().endswith(suffixes) and "-pred-" in f]
    for gen_fname in gen_files:
        gen_pattern = r'^(.+)-([^-]+)-pred-([^-]+)(?:-\d+)?\.png$'
        match = re.match(gen_pattern, gen_fname)
        if match:
            ...
            if key in gen_file_info_dict:
                ...
                continue
            gen_file_info_dict[key] = gen_fname
    print(f"Built index for {len(gen_file_info_dict)} valid generated images for GPT scoring")

    results = []

    for gt_fname in tqdm(gt_files, desc="GPT Scoring"):
        # 获取对应的生成文件名（传入预构建的索引字典）
        gen_fname = get_corresponding_gen_filename(gt_fname, gen_file_info_dict)
        if not gen_fname:
            print(f"[WARN] Could not find corresponding gen file for: {gt_fname} (skipped)")
            continue
        ...
        if not os.path.exists(gen_path):
            print(f"[WARN] Missing generated image: {gen_fname} (corresponding to {gt_fname})")
            continue
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"\n[ERROR] Failed to compute GPT score: {e}")
        print("Skip GPT scoring, please check API token and network connection.")